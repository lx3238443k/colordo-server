...
class UserService extends Service{
    ...
    async saveUserInfo(code,userInfo){
        ...
        // 调用微信接口获取openid
        let res = await this.ctx.curl(`https://api.weixin.qq.com/sns/jscode2session?appid=${appid}&secret=${secret}&js_code=${code}&grant_type=authorization_code`, { dataType: 'json' });
        ...
        const image =(await this.app.mysql.get('user',{openId:res.data.openid})).backgroundImage;
        const sql_res = await this.app.mysql.query(upsert,info);
        res.data['backgroundImage']=image;
        return res.data;
    }
